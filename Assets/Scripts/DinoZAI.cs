...
public class DinoZAI : MonoBehaviour {
		...
		if (!isDead) {
			if (!isFiredAt) {
				...
				if (this.gameObject.GetComponent<NavMeshAgent> ().remainingDistance > 0f) {
					...
					if (!idle) {
						this.gameObject.transform.GetChild(0).gameObject.GetComponent<Animator> ().SetBool ("isRunning", false);
						this.gameObject.transform.GetChild(0).gameObject.GetComponent<Animator> ().SetBool ("isIdle", true);
						this.gameObject.transform.GetChild(0).gameObject.GetComponent<Animator> ().SetBool ("isDead", false);
						this.gameObject.transform.GetChild(0).gameObject.GetComponent<Animator> ().SetBool ("isWalking", false);
						idle = true;
						die = walk = run = false;
					}
				...
				//running animation
				if (!run) {
					this.gameObject.transform.GetChild(0).gameObject.GetComponent<Animator> ().SetBool ("isRunning", true);
					this.gameObject.transform.GetChild(0).gameObject.GetComponent<Animator> ().SetBool ("isIdle", false);
					this.gameObject.transform.GetChild(0).gameObject.GetComponent<Animator> ().SetBool ("isDead", false);
					this.gameObject.transform.GetChild(0).gameObject.GetComponent<Animator> ().SetBool ("isWalking", false);
					run = true;
				...
				if (Vector3.Distance (this.gameObject.transform.position, safePoint.position) < 1f) {
					//game over
					GameManager.Instance.LoseGame();
					Debug.Log ("lose game function called");
			...
			if (!die) {
				this.gameObject.transform.GetChild(0).gameObject.GetComponent<Animator> ().SetBool ("isRunning", false);
				this.gameObject.transform.GetChild(0).gameObject.GetComponent<Animator> ().SetBool ("isIdle", false);
				this.gameObject.transform.GetChild(0).gameObject.GetComponent<Animator> ().SetBool ("isDead", true);
				this.gameObject.transform.GetChild(0).gameObject.GetComponent<Animator> ().SetBool ("isWalking", false);
				die = true;
	...
	void DestroyAnimation(){
		this.gameObject.transform.GetChild(0).gameObject.GetComponent<Animator> ().enabled = false;
